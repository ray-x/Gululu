...
class CDFlowLayout: UICollectionViewFlowLayout {
    ...
    override func targetContentOffset(forProposedContentOffset proposedContentOffset: CGPoint, withScrollingVelocity velocity: CGPoint) -> CGPoint {
        ...
        let horizontalCenter = proposedContentOffset.x + (collectionView!.bounds.width / 2.0)//collectionView落在屏幕中点的x坐标
        let targetRect = CGRect(x: proposedContentOffset.x, y: 0.0, width:  collectionView!.bounds.size.width, height: self.collectionView!.bounds.size.height)
...
//            attributes.transform3D = CATransform3DMakeRotation(zoom, 0, 0, zoom)
